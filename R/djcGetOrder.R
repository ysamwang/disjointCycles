...
#'
djcGetOrder <- function(Y, alpha2 = .01, alpha3 = .01, alphaR = .01, methodRoot = "indDelta", methodDet2 = "indDelta",
                           rescaleData = T, verbose = F){
...
djc_recur <- function(Y, alpha2 = .01, alpha3 = .01, alphaR = .01,
  ...
  # In both cases, the p-values (for each u) is adjusted using holm
  # singletonRoots should be a vector

  singletonRoots <- which(p.adjust(sapply(1:p, function(u){.test2joint(u, moments$sMat, moments$tMat, Y, method = methodRoot)$pval}),
                                   method = "holm") > alpha2)
