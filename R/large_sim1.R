...
p.list <- c(24)
n.list <- c(25, 50) * 1000
a.list <- c(2.5e-1, 1e-1, 1e-2, 1e-3)
mt.list <- c("BH", "holm")
...
rec <- matrix(0, rep.runs, 27)
for(i in 1:rep.runs){
  Lambda <- disjointCycles::cycleChain(p, cycleSize = cycleSize, lowEdge = lowEge, highEdge = highEdge,
...
colnames(rec) <- paste(rep(c("gamma", "mixNorm", "ln"), each = 7),
                       rep(c("order", "oracle_sen", "oracle_spc", "oracle_edges",
                             "est_sen", "est_spc", "est_edges", "timeOrd", "timeEdges"), times= 3),
                       sep = "_")
